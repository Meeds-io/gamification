...
if (extensionRegistry) {
  const components = extensionRegistry.loadComponents('EngagementCenterPrograms');
...
const appId = 'EngagementCenterPrograms';
...
export function init(isAdministrator, isProgramManager) {
  exoi18n.loadLanguageAsync(lang, urls).then(i18n => {
    ...
    Vue.createApp({
      ...
      template: `<engagement-center-programs id="${appId}" :is-administrator="${isAdministrator}" :is-program-manager="${isProgramManager}" class="card-border-radius overflow-hidden" />`,
      vuetify,
      i18n
