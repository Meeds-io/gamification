...
<script>
  ...
  export default {
    ...
    computed: {
    url() {
      if (!this.commonSpace || !this.commonSpace.groupId) {
        return '#';
      }
      const uri = this.commonSpace.groupId.replace(/\//g, ':');
      return `${eXo.env.portal.context}/g/${uri}/`;
    },
    avatarUrl() {
      return this.commonSpace && this.commonSpace.avatarUrl || `${eXo.env.portal.context}/${eXo.env.portal.rest}/v1/social/spaces/${this.commonSpace.avatarUrl}/avatar`;
    },
    labels() {
      return {
    ...
    mounted() {
      if (this.commonSpace.id && this.commonSpace.groupId) {
    ...
    initTiptip() {
      this.$nextTick(() => {
       $(`#${this.id}`).spacePopup({
          ...
          defaultAvatarUrl: this.commonSpace.avatarUrl,
