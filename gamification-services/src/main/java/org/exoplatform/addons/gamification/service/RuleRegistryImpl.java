...
import org.exoplatform.addons.gamification.model.RuleConfig;
...
public class RuleRegistryImpl implements Startable, RuleRegistry {
    ...
    try {
      ...
      for (RuleConfig rule : ruleMap.values()) {
        ...
        if (ruleDTO == null || !(ruleDTO.getEvent().equals(rule.getEvent()))
            || !(ruleDTO.getTitle().equals(GAMIFICATION_DEFAULT_DATA_PREFIX + rule.getTitle()))) {
          store(rule, ruleDTO);
        }
      }
    } catch (Exception e) {
  ...
  private boolean isRulesProcessingDone() {
    SettingValue<?> setting = CommonsUtils.getService(SettingService.class)
                                          .get(Context.GLOBAL,
                                               Scope.APPLICATION.id(GAMIFICATION_SETTINGS_RULE_KEY),
