...
public class GamificationRestEndpoint implements ResourceContainer {
  ...
  public Response getAllPointsByUserId(
    ...
    if (period == null || StringUtils.equalsIgnoreCase(Period.ALL.name(), period)) {
      earnedXP = realizationService.getScoreByIdentityId(identity.getId());
    } else {
      ...
      earnedXP = realizationService.getScoreByIdentityIdAndBetweenDates(identity.getId(), fromDate, toDate);
    }
    return Response.ok(new GamificationPoints().userId(userId)
                                               .points(earnedXP)
                                               .code("0")
   ...
   *
   * @param  userId         : user social id
   * @param  startDateEntry : Date from when gamification api filter earned
   *                          points
   * @param  endDateEntry   : Date until when gamification api filter eearned
   *                          points
   * @return                : and object of type GamificationPoints
   */
  @Path("points/date")
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @RolesAllowed("users")
  public Response getAllPointsByUserIdByDate(@QueryParam("userId")
  String userId, @QueryParam("startDate")
  String startDateEntry, @QueryParam("endDate")
  String endDateEntry) {
    if (StringUtils.isBlank(userId)) {
    ...
    try {
      ...
      Identity identity = identityManager.getOrCreateIdentity(OrganizationIdentityProvider.NAME, userId);
      Long earnedXP = realizationService.getScoreByIdentityIdAndBetweenDates(identity.getId(), startDate, endDate);
      return Response.ok(new GamificationPoints().userId(userId)
                                                 .points(earnedXP)
                                                 .code("0")
                                                 .message("Gamification API is called successfully"))
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @RolesAllowed("users")
  public Response getLeaderboardByDate(@Context
  UriInfo uriInfo,
    ...
    List<StandardLeaderboard> leaderboard = realizationService.getLeaderboardBetweenDate(IdentityType.getType(earnerType),
                                                                                         startDate,
                                                                                         endDate);
