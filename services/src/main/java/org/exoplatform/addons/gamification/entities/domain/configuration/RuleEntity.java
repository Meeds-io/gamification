...
@NamedQuery(name = "Rule.getHighestBudgetDomainIds", query = "SELECT rule.domainEntity.id FROM Rule rule WHERE rule.isEnabled = true AND rule.isDeleted = false AND (rule.domainEntity.audienceId in (:spacesIds) OR rule.domainEntity.audienceId = null ) GROUP BY rule.domainEntity.id ORDER BY SUM(rule.score) DESC")
