...
public class TestConnectorSettingsRest extends AbstractServiceTest { // NOSONAR

  protected Class<?> getComponentClass() {
    return ConnectorSettingsRest.class;
  }
  ...
  public void testGetConnectorsSettings() throws Exception {
    startSessionAs("user");
    ContainerResponse response = getResponse("GET", getURLResource("connectors/settings"), null);
    assertNotNull(response);
    assertEquals(401, response.getStatus());

    startSessionAsAdministrator("root1");
    response = getResponse("GET", getURLResource("connectors/settings"), null);
    assertNotNull(response);
  ...
  @Test
  public void testDeleteUserConnectorSettings() throws Exception {
    startSessionAs("user");
