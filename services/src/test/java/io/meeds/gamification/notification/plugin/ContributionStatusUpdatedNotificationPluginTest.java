...
public class ContributionStatusUpdatedNotificationPluginTest extends AbstractPluginTest { // NOSONAR
  ...
  public BaseNotificationPlugin getPlugin() {
    return pluginService.getPlugin(PluginKey.key(CONTRIBUTION_ACCEPTED_NOTIFICATION_ID));
  }
  ...
  public void testSimpleCase() throws Exception { // NOSONAR
    ...
    Announcement announcement = new Announcement(0,
            ...
            null,
            null);
    announcement = announcementService.createAnnouncement(announcement, null, SIMPLE_USER);

    realizationService.updateRealizationStatus(announcement.getId(), RealizationStatus.ACCEPTED);
    ...
    assertEquals(CONTRIBUTION_ACCEPTED_NOTIFICATION_ID, contributionAcceptedNotification.getKey().getId());
    NotificationContext ctx = NotificationContextImpl.cloneInstance();
    ctx.setNotificationInfo(contributionAcceptedNotification.setTo(SIMPLE_USER));
    MessageInfo info = buildMessageInfo(ctx);

    assertBody(info, rule.getTitle());
    assertEquals(SIMPLE_USER, contributionAcceptedNotification.getTo());
    ...
    realizationService.updateRealizationStatus(announcement.getId(), RealizationStatus.REJECTED);

    list = assertMadeWebNotifications(SIMPLE_USER, 2);
    NotificationInfo contributionRejectedNotification = list.get(1);

    assertEquals(CONTRIBUTION_REJECTED_NOTIFICATION_ID, contributionRejectedNotification.getKey().getId());
    ctx = NotificationContextImpl.cloneInstance();
    ctx.setNotificationInfo(contributionRejectedNotification.setTo(SIMPLE_USER));
    info = buildMessageInfo(ctx);

    assertBody(info, rule.getTitle());
    assertEquals(SIMPLE_USER, contributionRejectedNotification.getTo());
  }
